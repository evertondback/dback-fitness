...
async function history(env,url){
 const db=ensureDb(env),limit=Math.min(100,Math.max(1,Number(url.searchParams.get('limit')||30)));
 const sessions=await db.prepare('SELECT * FROM workout_sessions ORDER BY started_at DESC LIMIT ?').bind(limit).all();
 return json({ok:true,sessions:sessions.results||[]});
}
async function progress(env,url){
 const db=ensureDb(env),id=url.searchParams.get('exercise_id');if(!id)return json({error:'Missing exercise_id.'},400);const e=exerciseById(id);if(!e)return json({error:'Unknown exercise.'},404);
 const st=await state(env,id);const logs=await db.prepare('SELECT * FROM set_logs WHERE exercise_id=? ORDER BY completed_at DESC LIMIT 20').bind(id).all();return json({ok:true,exercise:e,state:st||null,recentSets:logs.results||[]});
}
