...
import {FEATURE_FLAGS} from './operating-contract.js';
...
import {handleV40Api,PLATFORM40_CSS,PLATFORM40_JS} from './v40-adaptive-platform.js';
import {APP41_CSS,APP41_JS} from './v41-fitness-app-shell.js';

function patch(html){
 let out=html;
 if(!FEATURE_FLAGS.unifiedShell) return out;
 for(const v of ['7.0.1','7.0.2','8.0.0','8.0.1','8.1.0','9.0.0','9.0.1','9.0.2','9.1.0','9.2.0','9.3.0','9.4.0','9.5.0','9.6.0','9.7.0','31.0.0','32.0.0','32.0.1','33.0.0','34.0.0','35.0.0','35.0.1','35.0.2','35.0.3','36.0.0','36.1.0','37.0.0','38.0.0','38.0.1','38.0.2','38.0.3','39.0.0','40.0.0'])out=out.replaceAll(v,SYSTEM_VERSION);
 out=out.replace(/Fitness OS\s*·\s*[0-9.]+/g,'Fitness OS · '+SYSTEM_VERSION);
 out=out.replace('</style>',CSS+HISTORY_CSS+RESPONSIVE_CSS+LAYOUT_HOTFIX_CSS+EQUIPMENT_LIBRARY_CSS+LIBRARY_INDEX_CSS+VIDEO_SOURCE_CSS+VIDEO_LIBRARY_CSS+VIDEO38_CSS+LIBRARY39_CSS+COLLAPSE_CSS+ANATOMY35_CSS+MOBILE35_CSS+COMPLETION31_CSS+TIMER37_CSS+OPS32_CSS+WARMUP33_CSS+COMMAND36_CSS+PLATFORM40_CSS+APP41_CSS+'</style>').replace('</body>','<script>'+CORE_JS+'</script><script>'+HISTORY_JS+'</script><script>'+RESPONSIVE_JS+'</script><script>'+EQUIPMENT_LIBRARY_JS+'</script><script>'+LIBRARY_INDEX_JS+'</script><script>'+VIDEO_SOURCE_JS+'</script><script>'+VIDEO_LIBRARY_JS+'</script><script>'+VIDEO38_JS+'</script><script>'+LIBRARY39_JS+'</script><script>'+COLLAPSE_JS+'</script><script>'+ANATOMY35_JS+'</script><script>'+MOBILE35_JS+'</script><script>'+COMPLETION31_JS+'</script><script>'+TIMER37_JS+'</script><script>'+NAV31_GUARD_JS+'</script><script>'+OPS32_JS+'</script><script>'+WARMUP33_JS+'</script><script>'+COMMAND36_JS+'</script><script>'+PLATFORM40_JS+'</script><script>'+APP41_JS+'</script></body>');
...
export default{async fetch(req,env,ctx){
 ...
 try{
  const v40=await handleV40Api(req,env,u);if(v40)return stamp(v40);
