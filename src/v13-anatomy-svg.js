...
.dbMuscleSvg .muscleRegion.secondarySelected{fill:rgba(59,130,246,.45);stroke:#bfdbfe}
...
.dbAtlasStatus{display:flex;justify-content:space-between;gap:8px;align-items:center;margin:7px 0 4px;font-size:11px;color:#8da8c2}.dbAtlasStatus b{color:#e5eef8}
...
@media(max-width:720px){.realAnatomyMap{aspect-ratio:400/560!important;max-height:none!important}.dbMuscleSvg .muscleRegion{stroke-width:1.1}.dbAtlasStatus{font-size:10px}.smartLabelRail{display:none!important}}
`;
...
const byId=id=>typeof anatById==='function'?anatById(id):null;
function setView(v){try{dbackAtlasView=v;localStorage.setItem('dback_atlas_view',v)}catch{}document.querySelectorAll('#view-anatomy .bodyPanel').forEach(p=>p.classList.toggle('atlasActive',p.dataset.atlasView===v));document.querySelectorAll('.dbAtlasViewToggle button').forEach(b=>b.classList.toggle('active',b.dataset.v===v));}
function toggleMuscle(id){if(typeof anatMuscles==='undefined')return;if(anatMuscles.has(id))anatMuscles.delete(id);else anatMuscles.add(id);localStorage.setItem('dback_anat_muscles',JSON.stringify([...anatMuscles]));if(typeof renderAnatomy==='function')renderAnatomy();setTimeout(upgrade,40)}
function makePath(svg,id,d){const p=document.createElementNS(NS,'path');p.setAttribute('d',d);p.setAttribute('class','muscleRegion'+(anatMuscles?.has(id)?' selected':''));p.setAttribute('data-muscle',id);p.setAttribute('tabindex','0');const m=byId(id);p.setAttribute('aria-label',m?.name||id);p.addEventListener('click',()=>toggleMuscle(id));p.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();toggleMuscle(id)}});svg.appendChild(p)}
function buildMap(panel,view){const map=panel.querySelector('.realAnatomyMap');if(!map||map.querySelector('.dbMuscleSvg'))return;map.querySelectorAll('.mapHotspot,.mapJoint').forEach(x=>x.style.display='none');const svg=document.createElementNS(NS,'svg');svg.setAttribute('viewBox','0 0 400 560');svg.setAttribute('preserveAspectRatio','xMidYMid meet');svg.setAttribute('class','dbMuscleSvg');svg.setAttribute('role','group');svg.setAttribute('aria-label',view+' selectable muscle map');const defs=document.createElementNS(NS,'defs');defs.innerHTML='<filter id="dbglow"><feGaussianBlur stdDeviation="2.2" result="b"/><feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge></filter>';svg.appendChild(defs);const data=view==='front'?FP:BP;Object.entries(data).forEach(([id,paths])=>paths.forEach(d=>makePath(svg,id,d)));map.appendChild(svg)}
function selectedRail(view){const panel=[...document.querySelectorAll('#view-anatomy .bodyPanel')].find(p=>p.dataset.atlasView===view);if(!panel)return;panel.querySelector('.dbSelectedMuscles')?.remove();const ids=[...(anatMuscles||[])].filter(id=>{const m=byId(id);return m&&(m.view===view||m.view==='both')});const r=document.createElement('div');r.className='dbSelectedMuscles';if(!ids.length){r.innerHTML='<span class="muted">Tap directly on a muscle to select it.</span>'}else ids.forEach(id=>{const b=document.createElement('button');b.type='button';b.textContent=(byId(id)?.name||id)+' ×';b.onclick=()=>toggleMuscle(id);r.appendChild(b)});panel.querySelector('.realAnatomyMap')?.after(r)}
function toolbar(view){const root=document.getElementById('view-anatomy');if(!root||root.querySelector('.dbAtlasViewToggle'))return;const t=document.createElement('div');t.className='dbAtlasViewToggle';t.innerHTML='<button type="button" data-v="front">FRONT</button><button type="button" data-v="back">BACK</button>';const anchor=root.querySelector('.atlasToolbar')||root.querySelector('.bodyGrid');anchor?.before(t);t.querySelectorAll('button').forEach(b=>b.onclick=()=>setView(b.dataset.v));setView(view)}
function upgrade(){const root=document.getElementById('view-anatomy');if(!root)return;const panels=[...root.querySelectorAll('.bodyPanel')];if(!panels.length)return;panels.forEach((p,i)=>{const v=p.dataset.atlasView||(i===0?'front':'back');p.dataset.atlasView=v;buildMap(p,v);selectedRail(v)});toolbar(typeof dbackAtlasView==='string'?dbackAtlasView:'front');setView(typeof dbackAtlasView==='string'?dbackAtlasView:'front')}
const old=typeof renderAnatomy==='function'?renderAnatomy:null;if(old){renderAnatomy=function(){old();requestAnimationFrame(upgrade)}}
document.addEventListener('click',e=>{const tx=e.target.closest('button')?.textContent?.trim();if(tx==='Anatomy Lab'||tx==='Anatomy')setTimeout(upgrade,180)},true);new MutationObserver(()=>{clearTimeout(window.__dbAnat13);window.__dbAnat13=setTimeout(upgrade,80)}).observe(document.body,{subtree:true,childList:true});setTimeout(upgrade,650);
})();`;
